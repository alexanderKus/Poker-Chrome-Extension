...
chrome.alarms.onAlarm.addListener(async (alarm) => {
    ...
    const { roundLength, timer, isRunning } = await chrome.storage.sync.get(["roundLength", "timer", "isRunning"])
    if (! isRunning) return
    if (timer === roundLength * 60) {
        ...
        const { smallBlind, bigBlind, maxSmallBlind, maxBigBlind, factor } =
            await chrome.storage.sync.get(["smallBlind", "bigBlind", "maxSmallBlind", "maxBigBlind", "factor"])
        chrome.storage.sync.set({
            timer: 0,
            smallBlind: Math.max(smallBlind * factor, maxSmallBlind),
            bigBlind: Math.max(bigBlind * factor, maxBigBlind)
        })
        return
    }
...
chrome.storage.sync.get(["timer", "isRunning", "roundLength", "smallBlind", "bigBlind", "maxSmallBlind", "maxBigBlind", "factor"], (res) => {
    chrome.storage.sync.set({
